...
// -----------------------------------------
//
// Source: tempNull.java
// Date  : 12 Feb 2017 09:55:53 ECT
 ...
 * Automatically generated load test program.
 */
public class tempNull extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	private static final String EXTERNAL_PARAM_FILE = "tempNullExtParams.dat";              // input file name for very large parameter values
	 ...
	 */
	public tempNull(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0002 = "detectportal.firefox.com";
		int    requestPort0002 = 80;
		String requestFile0002 = "/success.txt";
		...
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/PLAIN", null, null);		// Test [2] <- Index 2
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestHost0003 = "bankgirot.se";
		...
		String requestHost0004 = "www.googletagmanager.com";
		int    requestPort0004 = 80;
		String requestFile0004 = "/gtm.js" +
				"?id=GTM-P7LNJ5";
		...
		String requestFile0005 = "/globalassets/bilder/sajtbakgrunder/bg-rullband.png";
		...
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {304}, null, null, null);		// Test [5] <- Index 5
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
		...
		String requestFile0006 = "/globalassets/bilder/toppbanner_start/bg_montage_varlden_824x342.gif";
		...
		// verify response: status code = 304, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 0
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {304}, null, null, null);		// Test [6] <- Index 6
		if (!urlCallPassed)
		{
			// failure - dump wrong content to thread log and abort current loop
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("tempNull", concurrentUsers);
		...
		performanceData.setInfoText("tempNull");
		...
		for (int x = 0; x < concurrentUsers; x++)
		{
			...
			try
			{
				...
				tempNull simulatedUser = new tempNull(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
